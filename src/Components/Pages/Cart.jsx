  ...
  return (
    <div className='w-full pt-[120px] md:pt-[60px] ' >
     ...
     :<div className='w-full min-h-screen px-2 flex flex-col-reverse md:flex-row gap-3 justify-around'>
      ...
      {cartproducts?.map((a)=>(
      <div className='w-full mb-5 py-2 h-[250px] gap-2 text-blue-700 shadow-md shadow-gray-300 flex' key={a.id}>
        <img className='w-[40%] h-full' src={a.image} />
         <div className='font-bold relative space-y-2'>
          <p className='text-2xl'>{a.title.slice(0,35)}</p>
      ...
      <div onClick={()=>dispatch(removecart(a))} className='w-14  place-items-center   h-7 rounded-full text-xl bg-white shadow-md
